...
/// and a symbol name or other std::string token naming the object.
class CPoolRecord {
public:
  /// \brief Generic constant pool tag types
  enum {
    primitive=0,	///< Constant \b value of data-type \b type, cpool operator can be eliminated
    string_literal=1,	///< Constant reference to std::string (passed back as \b byteData)
...
private:
  ...
  uint1 *byteData;	///< For std::string literals, the raw byte data of the std::string
  int4 byteDataLen;	///< The number of bytes in the data for a std::string literal
public:
  ...
  const uint1 *getByteData(void) const { return byteData; }			///< Get pointer to std::string literal data
  int4 getByteDataLength(void) const { return byteDataLen; }			///< Number of bytes of std::string literal data
  ...
  void saveXml(std::ostream &s) const;						///< Save object to an XML stream
...
public:
  ...
  const CPoolRecord *restoreXmlRecord(const std::vector<uintb> &refs,const Element *el,TypeFactory &typegrp);
...
///
/// The CPoolRecord objects are held directly in a std::map container. This class can be used
...
class ConstantPoolInternal : public ConstantPool {
  ...
  /// container than an open-ended std::vector.
  ...
  public:
    ...
    void saveXml(std::ostream &s) const;		///< Serialize the \e reference to an XML element
    void restoreXml(const Element *el);		///< Deserialize the \e reference from an XML element
  };
  std::map<CheapSorter,CPoolRecord> cpoolMap;	///< A std::map from \e reference to constant pool record
