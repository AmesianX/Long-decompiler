...
class NameSymbol : public ValueSymbol {
  vectorstd::string nametable;
  bool tableisfilled;
  void checkTableFill(void);
public:
  NameSymbol(void) {}		// For use with restoreXml
  NameSymbol(const std::string &nm,PatternValue *pv,const vectorstd::string &nt) : ValueSymbol(nm,pv) { nametable=nt; checkTableFill(); }
  virtual Constructor *resolve(ParserWalker &walker);
  virtual void print(std::ostream &s,ParserWalker &walker) const;
...
class ContextOp : public ContextChange {
  ...
  int4 num;			// index of word containing context variable to std::set
...
class DecisionProperties {
  vectorstd::string identerrors;
  vectorstd::string conflicterrors;
public:
  void identicalPattern(Constructor *a,Constructor *b);
  void conflictingPattern(Constructor *a,Constructor *b);
  const vectorstd::string &getIdentErrors(void) const { return identerrors; }
  const vectorstd::string &getConflictErrors(void) const { return conflicterrors; }
};

class DecisionNode {
  std::vector<pair<DisjointPattern *,Constructor *> > std::list;
