...
/// Print a std::dec dump of a data buffer to stream
extern void print_data(std::ostream &s,uint1 *buffer,int4 size,const Address &baseaddr);
//extern void print_char(std::ostream &s,int4 onechar);
//extern bool print_string(std::ostream &s,uint1 *buffer,int4 size);
...
/// Convert std::string to type \b meta-type
...
public:
  ...
  Datatype(int4 s,type_metatype m,const std::string &n) { name=n; size=s; metatype=m; flags=0; id=0; }
  ...
  virtual void saveXml(std::ostream &s) const;	///< Serialize the data-type to XML
  int4 typeOrder(const Datatype &op) const { if (this==&op) return 0; return compare(op,10); }	///< Order this with -op- datatype
  void saveXmlBasic(std::ostream &s) const;	///< Save basic data-type properties
  void saveXmlRef(std::ostream &s) const;	///< Write an XML reference of \b this to stream
};
...
/// A std::set of data-types sorted by function
typedef std::set<Datatype *,DatatypeCompare> DatatypeSet;

/// A std::set of data-types sorted by name
...
public:
  ...
  TypeBase(int4 s,type_metatype m,const std::string &n) : Datatype(s,m,n) {}
...
public:
  ...
  TypeChar(const std::string &n) : TypeBase(1,TYPE_INT,n) { flags |= Datatype::chartype; }
...
class TypeUnicode : public TypeBase { // Unicode character type
  void setflags(void);	///< std::set unicode property flags
protected:
  friend class TypeFactory;
...
/// This should be the only object with meta-type std::set to TYPE_VOID
...
protected:
  ...
  void setNameMap(const std::map<uintb,std::string> &nmap);	///< Establish the value -> name std::map
  virtual void restoreXml(const Element *el,TypeFactory &typegrp);
public:
  ...
  TypeEnum(int4 s,type_metatype m,const std::string &nm) : TypeBase(s,m,nm) { flags |= enumtype; }
  std::map<uintb,std::string>::const_iterator beginEnum(void) const { return namemap.begin(); }	///< Beginning of name std::map
  std::map<uintb,std::string>::const_iterator endEnum(void) const { return namemap.end(); }	///< End of name std::map
...
protected:
  friend class TypeFactory;
  std::vector<TypeField> field;			///< The std::list of fields
  void setFields(const std::vector<TypeField> &fd);	///< Establish fields for \b this
  int4 getFieldIter(int4 off) const;		///< Get index into field std::list
  virtual void restoreXml(const Element *el,TypeFactory &typegrp);
public:
  TypeStruct(const TypeStruct &op);	///< Construct from another TypeStruct
  TypeStruct(const std::string &n) : Datatype(0,TYPE_STRUCT,n) {}	///< Construct empty TypeStruct from a name
...
public:
  ...
  TypeCode(const std::string &nm);		///< Construct from a name
  ...
  void setProperties(bool hasThisPtr,bool isConstructor,bool isDestructor);	///< std::set additional function properties
...
class TypeFactory {
  ...
  void orderRecurse(std::vector<Datatype *> &deporder,DatatypeSet &mark,Datatype *ct) const;	///< Write out dependency std::list
...
protected:
  ...
  virtual Datatype *findById(const std::string &n,uint8 id);		///< Search by name and id
public:
  ...
  void setStructAlign(int4 al) { align = al; }		///< std::set the default structure alignment
  ...
  Datatype *setName(Datatype *ct,const std::string &n); 	///< std::set the given types name
  bool setFields(std::vector<TypeField> &fd,TypeStruct *ot,int4 fixedsize);	///< std::set fields on a TypeStruct
  bool setEnumValues(const std::vector<std::string> &namelist,
		      const std::vector<uintb> &vallist,
		      const std::vector<bool> &assignlist,
		      TypeEnum *te);		///< std::set named values for an enumeration
  ...
  void saveXml(std::ostream &s) const;			///< Save \b this container to stream
  void saveXmlCoreTypes(std::ostream &s) const;		///< Save core types to stream
