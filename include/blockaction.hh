...
///
/// Ultimately this produces a std::list of \e likely \e gotos, which is used whenever the structuring
...
class TraceDAG {
  ...
  struct BlockTrace {
	...
	std::list<BlockTrace *>::iterator activeiter; ///< Position of \b this in the active trace std::list
  ...
  std::list<FloatingEdge> &likelygoto;	///< A reference to the std::list of likely goto edges being produced
  std::vector<FlowBlock *> rootlist;		///< List of root FlowBlocks to trace from
  std::vector<BranchPoint *> branchlist;	///< Current std::set of BranchPoints that have been traced
  ...
  std::list<BlockTrace *> activetrace;	///< The std::list of \e active BlockTrace objects
...
class CollapseStructure {
  ...
  bool likelylistfull;				///< Have we generated a \e likely \e goto std::list for the current innermost loop
  std::list<FloatingEdge> likelygoto;		///< The current \e likely \e goto std::list
  std::list<FloatingEdge>::iterator likelyiter;	///< Iterator to the next most \e likely \e goto edge
  std::list<LoopBody> loopbody;			///< The std::list of loop bodies for this control-flow graph
