...
/// only be determined from context, whether the edge is in the incoming or outgoing edge std::list.
struct BlockEdge {
  ...
  void saveXml(std::ostream &s) const;	///< Save the edge to an XML stream
...
public:
  ...
  /// are removed from the std::list of primary edges. These flags keep track only of
...
private:
  ...
  static void replaceEdgeMap(std::vector<BlockEdge> &vec);	///< Update block references in edges with copy std::map
...
public:
  ...
  virtual void saveXmlHeader(std::ostream &s) const;		///< Save basic information as XML attributes
  virtual void restoreXmlHeader(const Element *el);	///< Restore basic information for XML attributes
  virtual void saveXmlBody(std::ostream &s) const {}		///< Save detail about components to an XML stream
  ...
  void saveXmlEdges(std::ostream &s) const;			///< Save edge information to an XML stream
  void restoreXmlEdges(List::const_iterator &iter,List::const_iterator enditer,BlockMap &resolver);
  void saveXml(std::ostream &s) const;			///< Write out \b this to an XML stream
  ...
  static block_type nameToType(const std::string &name);	///< Get the block_type associated with a name std::string
  static std::string typeToName(block_type bt);		///< Get the name std::string associated with a block_type
...
/// A std::set of control-flow blocks can be grouped together and viewed as a single block,
...
class BlockGraph : public FlowBlock {
  ...
  void clearEdgeFlags(uint4 flags);	///< Clear a std::set of properties from all edges in the graph
...
public:
  ...
  virtual void saveXmlBody(std::ostream &s) const;
  virtual void restoreXmlBody(List::const_iterator &iter,List::const_iterator enditer,BlockMap &resolver);
...
/// As decompiler transformations progress, the std::set of addresses associated with the current std::set of
...
/// std::list of FlowBlock objects sorted by index and then looks up the FlowBlock matching a given
...
public:
  ...
  void sortList(void);					///< Sort the std::list of FlowBlock objects
...
///
/// Find the std::set of PcodeOp objects that need to be adjusted to flip
